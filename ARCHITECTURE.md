...
| `BinderSlot` | 卡冊頁面上的單一**格位**，記錄該格的卡、狀態、使用者自訂標籤，以及所屬的跨格群組（若有）。 |
| `BinderSlotGroup` | 一組相鄰格位，共同呈現同一張來源圖的各自區塊（見下方「跨格群組」）。 |

### 兩層收藏模型

標記（`UserCard`）與格位（`BinderSlot`）是兩層獨立資料：

- 卡牌必須先有 `UserCard` 才能放入格位。格位以 `(pageNumber, slotIndex)` 定位，且為**稀疏資料**——
  格線上未使用到的位置不會有 row，已清空的位置則保留為 `cardId` 為 null 的空格 row（放入新卡時優先重用）。
...
少數官方卡牌屬於「需組合才成一張場上卡」的家族：資料來源只提供**一個卡片頁與一張合成圖**，
實體卻是分開印刷的數張卡。這類卡在 `Card` 維持**單列**——拆成多列必須裁切官方合成圖（不做），
或讓多個格位顯示同一張圖。

卡冊內則依實體佔 N 格：`BinderSlotGroup` 描述群組的 `cols` × `rows` 與 `rotation`，成員格位以
`BinderSlot.groupId` / `groupIndex` 標記，各格以 CSS 定位呈現合成圖的對應區塊，**不產生任何裁切後的圖片檔**。

- 群組內**每格都是自足的完整 row**（各自帶 `cardId` / `status`）。既有「找空格」查詢（`cardId: null`）
  與 unique constraint 因此天然正確，無需感知群組；代價是數量與統計必須排除 `groupIndex > 0` 的成員，
  否則一組會被算成 N 張。
- 刪除、狀態切換與拖曳皆以整組為單位。卡冊格式（格線）變更時盡力保留群組矩形，放不下才退回單格。

## 認證與安全設計

- **強制 email 驗證**：email/password 註冊後需點驗證信連結才可登入；社群登入（OAuth）不受此限制。
